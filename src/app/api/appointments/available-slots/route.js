...
export async function GET(req) {
  ...
  const breakStartMinutes = hasBreak ? timeToMinutes(workingHour.breakStart) : null;
  const breakEndMinutes = hasBreak ? timeToMinutes(workingHour.breakEnd) : null;
  ...
  while (currentMinutes + service.duration <= endMinutes) {
    ...
    const overlapsBreak =
      hasBreak && currentMinutes < breakEndMinutes && slotEndMinutes > breakStartMinutes;
    ...
    const isTaken = existingAppointments.some((a) => a.startTime === slotStart);
